...
class OneThroughThreeDialog : public wxDialog
{
	...
	OneThroughThreeDialog(wxWindow* parent, wxWindowID,
		const wxBitmap&, const wxPoint&, const Movie*, const std::string* key, const Trackee*);
	...
	wxBitmap subBitmap;
	wxPoint relativePoint; // in coordinates relative to the origin (top-left) of the sub bitmap
...
void oneThroughThree(const wxBitmap& bitmap, const wxPoint& point, const Movie* movie, const std::string* key,
	const Trackee* trackee)
{
	...
	MyDialog* myDialog = new MyDialog{nullptr, wxID_ANY, subBitmap, wxPoint{point.x - topLeft.x, point.y - topLeft.y},
	                                  movie, key, trackee};
...
OneThroughThreeDialog::OneThroughThreeDialog(wxWindow* parent, wxWindowID id, const wxBitmap& subBitmap,
	const wxPoint& relativePoint, const Movie* movie, const std::string* key,
	const Trackee* trackee) :
		wxDialog{parent, id, "OneToThree", wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE},
		...
		movie{movie}, trackeeKey{key}, trackee{trackee}
{
	...
	if (buttonSizer) {
		topSizer->Add(CreateSeparatedSizer(buttonSizer), 0, wxEXPAND | wxALIGN_RIGHT | wxALL, 8);
	}
...
double OneThroughThreeDialog::averageAnnulus(const Bitmap& bitmap, const Point& center)
{
	...
	for (unsigned row = firstRow; row <= lastRow; ++row)
	{
		for (unsigned column = firstColumn; column <= lastColumn; ++column)
		{
			...
			if (squaredDistance < std::pow(outerRadius, 2) && // is this point inside the outer circle? If so,
			    squaredDistance >= std::pow(innerRadius, 2))  // is it also outside of the inner circle?
			{
				++pixels;
	...
	if (track)
	{
		...
		for (std::size_t i = 0; i < movie->getSize(); ++i)
		{
			...
			if ((*track)[i] != Point{-1, -1})
			{
				...
				oStream << '\t' << static_cast<int>((*bitmap)[(*track)[i].y][(*track)[i].x]); // gray level
						...
						if (iStream)
						{
							...
							if (key == "unwrapped_range") {
								...
								if (iStream) {
									...
									oStream << '\t' << std::fixed << std::setprecision(4) << unwrappedRange;
								}
							}
							iStream.close();
						}
...
void OneThroughThreeDialog::onImagePanelPaint(wxPaintEvent&)
{
	...
	if (gC)
	{
		...
		//int radius = slider[0]->GetValue(); // signed to avoid underflow of the expression...
		//gC->DrawEllipse(relativePoint.x - radius, relativePoint.y - radius, 2 * radius, 2 * radius); // ...here
		//radius = slider[1]->GetValue();
		//gC->DrawEllipse(relativePoint.x - radius, relativePoint.y - radius, 2 * radius, 2 * radius);
