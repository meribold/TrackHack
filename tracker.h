...
class Tracker
{
	...
	// the last parameter denotes the auxiliaryPoints distance (in frames) to the Bitmap parameter
	Point trackDown(Trackee&, std::shared_ptr<const Bitmap>, const Point& adjacentPoint, const Point& auxiliaryPoint,
	                unsigned proximity);
...
inline void Tracker::track(Trackee& trackee, const Movie& movie)
{
	...
	if (last != track->end())
	{
		...
		{
			--i; *i = trackDown(trackee, movie.getFrame(i - track->begin()).getBitmap(), *(i + 1));
		}
		...
		while (last != track->end())
		{
			...
			while (first != i)
			{
				*first = trackDown(trackee, movie.getFrame(first - track->begin()).getBitmap(), *(first - 1), *i,
					i - first);
				++first;
				if (first != i) {
					--i;
					*i = trackDown(trackee, movie.getFrame(i - track->begin()).getBitmap(), *(i + 1), *(first - 1),
						i - first + 1);
				} else { break; }
			}
			first = std::find(last, track->end(), Point{-1, -1});
		...
		{
			*first = trackDown(trackee, movie.getFrame(first - track->begin()).getBitmap(), *(first - 1));
		}
	}
}

inline Point Tracker::trackDown(Trackee& trackee, std::shared_ptr<const Bitmap> bitmap, const Point& adjacentPoint)
{
	std::size_t firstRow    = unsigned{adjacentPoint.y} < trackee.speedCap ? 0 : adjacentPoint.y - trackee.speedCap;
	std::size_t lastRow     = adjacentPoint.y + trackee.speedCap < bitmap->height ? adjacentPoint.y + trackee.speedCap :
	                                                                                bitmap->height - 1;
	std::size_t firstColumn = unsigned{adjacentPoint.x} < trackee.speedCap ? 0 : adjacentPoint.x - trackee.speedCap;
	std::size_t lastColumn  = adjacentPoint.x + trackee.speedCap < bitmap->width ? adjacentPoint.x + trackee.speedCap :
	                                                                               bitmap->width - 1;
	...
	for (std::size_t row = firstRow; row <= lastRow; ++row)
	{
		...
		{
			squaredDisplacement = std::pow(adjacentPoint.x - int{column}, 2) + std::pow(adjacentPoint.y - int{row}, 2);
			if (squaredDisplacement <= squaredSpeedCap)
			{
				if ((*bitmap)[row][column] > intensityPeak)
				{
					...
					std::pow(adjacentPoint.x - peakPoint.x, 2) + std::pow(adjacentPoint.y - peakPoint.y, 2))
				{
					peakPoint = Point{column, row};
				}
			}
                                ...
                                const Point& auxiliaryPoint, unsigned proximity)
{
	...
	for (std::size_t row = firstRow; row <= lastRow; ++row)
	{
		for (std::size_t column = firstColumn; column <= lastColumn; ++column)
		{
			...
			if (squaredDisplacement <= squaredSpeedCap)
			{
				int contendersNiceness = niceness(Point{column, row}, (*bitmap)[row][column], adjacentPoint,
					auxiliaryPoint, trackee.speedCap, trackee.speedCap * proximity);
...
inline int Tracker::niceness(const Point& point, unsigned char intensity, const Point& adjacentPoint,
	const Point& auxiliaryPoint, unsigned speedCap, unsigned distanceCap)
{
	double squaredDistance    = std::pow(point.x - auxiliaryPoint.x, 2) + std::pow(point.y - auxiliaryPoint.y, 2);
	...
	// when sqaredDistance approaches squaredDistanceCap a full proximityBonus is given and a pitch-black
	// pixel can be as good as a white one and vice versa;
	// if squaredDistance is higher then squaredDistanceCap the user should reconsider the speed cap
	int proximityBonus = 255. * gainedDistance / speedCap * squaredDistance / squaredDistanceCap;
