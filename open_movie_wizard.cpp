...
class OpenMovieWizard::DirDialogPage : public wxWizardPageSimple
{
	...
	DirDialogPage(OpenMovieWizard* parent, wxString* dir = nullptr, const wxFileHistory* dirHistory = nullptr);
...
OpenMovieWizard::OpenMovieWizard(wxWindow* parent, int id, wxString* dir, wxString* regEx, wxConfigBase* config,
	...
	regExPage{new RegExCtrlPage{this, regEx, config}}
{
	...
	// here, "this will enlarge the wizard to fit the biggest page"; see the documentation of
	GetPageAreaSizer()->Add(dirPage); // wxWizard::GetPageAreaSizer() in the wxWizard Class Reference
}
...
OpenMovieWizard::DirDialogPage::DirDialogPage(OpenMovieWizard* parent, wxString* dir, const wxFileHistory* dirHist) :
	wxWizardPageSimple{parent}
{
	//// <_configuration_information_handling_> ////
	///
	// note that the wizard itself does not store any directories; it could if it would save the
	// wxConfig object (that might be reasonable) - this page doesn't store them either (tho, I guess,
	// it could do so from its destructor)

	// has, as per its documentation, the "full set of std::vector<wxString> compatible methods";
	wxArrayString dirs{}; // I'll therefore use it as if it were a vector
	...
	// if possible, have the validator of the combo box transfer the most recent directory to it when
	if (!dirs.empty()) *dir = dirs[0]; // wxWindow::InitDialog() is called (i.e. as the dialog is being shown)
	///
	//// </_configuration_information_handling_> ////
	...
	hBox->Add(new wxStaticText{this, wxID_ANY, "Your directory", wxDefaultPosition, wxDefaultSize, wxALIGN_CENTER},
	...
	topSizer->Add(comboBox = new wxComboBox{this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, dirs,
		0, DirValidator{dir}}, 0, wxEXPAND);
	...
	Bind(wxEVT_COMMAND_BUTTON_CLICKED, &OpenMovieWizard::DirDialogPage::onButton, this, wxID_ANY);
}
...
OpenMovieWizard::RegExCtrlPage::RegExCtrlPage(OpenMovieWizard* parent, wxString* regEx, wxConfigBase* config)
	: wxWizardPageSimple{parent}
{
	wxControl* regExText;
	...
	if (config)
	{
		...
		{
			wxString key;
			long     cookie; // this confusing variable is passed to the wxConfigBase enumeration functions
			                 // by non-const reference; presumably in an attempted bribery

			if (config->GetFirstEntry(key, cookie)) // note that apparently config->GetFirstEntry() and ...
			{
				regExes.push_back(config->Read(key));
				while (config->GetNextEntry(key, cookie)) // ... config->GetNextEntry() retrieve keys
	...
	if (!helpURL.empty()) {
		regExText = new wxHyperlinkCtrl{this, wxID_ANY, u8"(Perl-derived) Regular Expression", helpURL};
	} else {
	...
	topSizer->Add(new wxStaticText{this, wxID_ANY,
		...
		"left empty."});
	topSizer->Add(0, 0, 1);
	topSizer->Add(new wxStaticText{this, wxID_ANY, "Your regular expression"}, 0, wxALIGN_CENTER);
	topSizer->Add(comboBox = new wxComboBox{this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, regExes,
		0, wxTextValidator{wxFILTER_NONE, regEx}}, 0, wxEXPAND);
	topSizer->Add(0, 0, 1);
...
bool OpenMovieWizard::DirDialogPage::DirValidator::Validate(wxWindow* parent)
{
	...
	if (!isValid) {
		...
		msg = u8"'" + window->GetValue() +
				u8"' should be a directory that this process has read and write permissions on.";
		isValid = false;
	}

	if (!isValid) ::wxMessageBox(msg, u8"Validation conflict", wxOK | wxICON_EXCLAMATION, parent);
