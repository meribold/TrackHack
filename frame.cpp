...
std::shared_ptr<const Bitmap> Frame::getBitmap(bool load) const
{
	...
	if (!bitmap && load)
	{
		// code that does not allocate and initialize memory for a temporary RGB image would be potentially faster
		// and more elegant;
		// the CImg or OpenCV library might constitue a suitable replacement for wxWidgets image loading facilities
	...
	return bitmap; // nullptr if the bitmap wasn't loaded already and the load flag was explicitly set to false
}
